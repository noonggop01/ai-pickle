...
Run `/cost` after a simple "fix this bug" request and the overhead portion is easy to spot: it's the chunk of input tokens billed before your actual instruction, file contents, or the model's reasoning even factor in.
...
In my own use, this shows up fastest in long single sessions — keep one conversation going for a while, with lots of back-and-forth, and token consumption noticeably accelerates as the context grows, well before you'd expect to feel it. That's the overhead problem compounding in real time: it's not a flat per-session cost, it's one that gets heavier the longer the session runs.
...
|---|---|---|
| Reported baseline token overhead | ~33k tokens per request (as reported; verify against your own `/cost` output) | ~7k tokens per request (as reported; varies by config) |
...
| Pricing model | Subscription (Claude Pro/Max) or API metering — check current tiers on Anthropic's pricing page | Open source, bring your own API key |
...
Anecdotally, this tends to track with how much scaffolding a tool ships by default: heavier default tooling correlates with fewer surprise permission prompts and destructive-edit retries on multi-file refactors, while leaner setups put more of that burden on how carefully you've configured the model and tools yourself.
...
To some extent — disabling unused tools and trimming memory files (like CLAUDE.md) can shave off some tokens, but the core system prompt isn't something most users can rewrite from scratch.
