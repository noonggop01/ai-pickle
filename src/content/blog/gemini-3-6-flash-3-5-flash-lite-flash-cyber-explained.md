...
- Pricing positioned below Gemini Pro but above Flash-Lite [SOURCE NEEDED]
...
[EXPERIENCE: note here if you ran the same prompt set against the old Flash model and 3.6 Flash and saw output style/formatting drift]
...
[EXPERIENCE: describe a specific security-analysis prompt tested against Flash Cyber vs 3.6 Flash and how the outputs differed]
...
| Gemini 3.5 Flash Cyber | Security/threat analysis, SOC tooling | Medium (likely priced near 3.6 Flash) [SOURCE NEEDED] | Fast | High, domain-tuned |
...
Treat the cost and speed columns as directional rather than exact — Google's official pricing page is the source of truth and it changes more often than model names do. [SOURCE NEEDED]
...
[EXPERIENCE: mention actual latency/cost numbers observed when swapping a production workload between two of these models]
...
This isn't confirmed publicly in detail yet. [SOURCE NEEDED] Check Google's model access page or your Vertex AI console for actual availability, since specialized models sometimes roll out to enterprise tiers before general API access.

**Will pricing differ significantly between the three models?**
Almost certainly yes, given the naming pattern Google uses (Flash vs Flash-Lite has always meant a real price gap). Exact numbers should come from Google's official pricing page rather than assumption. [SOURCE NEEDED]
